  ...
  fastify.get('/role', async (request, reply) => {
    try {
      ...
      if (role === 'supervisor' || role === 'admin') {
        const [paraCapacity, overdueAll, staleCases, queueBottlenecks, weeklyVolume] = await Promise.all([
        ...
        return { role, paralegal_capacity: paraCapacity.rows, overdue_all: overdueAll.rows, stale_cases: staleCases.rows, queue_bottlenecks: queueBottlenecks.rows, weekly_volume: weeklyVolume.rows[0] || {} };
      }
